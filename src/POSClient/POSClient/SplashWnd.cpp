...
unsigned int __stdcall CSplashWnd::SplashThreadProc( void* lpParameter )
{
  ...
  else
  {
	...
	//
	pThis->m_hSplashWnd = CreateWindowEx(WS_EX_TOOLWINDOW| WS_EX_TOPMOST , L"SplashWnd", pThis->m_WindowName.c_str(), 
	...
	SetWindowLongPtr(pThis->m_hSplashWnd, -21, reinterpret_cast<LONG_PTR>(pThis) );
...
{
	CSplashWnd* pInstance = reinterpret_cast<CSplashWnd*>(GetWindowLongPtr(hwnd, -21));
	if (pInstance == NULL)
	{
  ...
  switch (uMsg)
	{
		case WM_PAINT:
		{
			if (pInstance->m_pImage)
			{
				...
				{
					Gdiplus::Font msgFont( _T("Microsoft YaHei"), 12,Gdiplus::FontStyleRegular, Gdiplus::UnitPixel );
...
void CSplashWnd::SetProgress( UINT procent, UINT nResourceID, HMODULE hModule )
{
	...
	theLang.LoadString(nResourceID,msg);
	std::wstring* tempmsg = new std::wstring( msg, msg.GetLength() );
	PostThreadMessage( m_ThreadId, PBM_SETPOS, procent, reinterpret_cast<LPARAM>(tempmsg) );
}

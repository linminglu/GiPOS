...
void CACEdit::ReadDirectory(CString m_Dir)
{
	...
	if (m_Dir.Right(1) != _T('\\'))
		{
		_wsplitpath(m_Dir, m_szDrive, m_szDir, m_szFname, m_szExt);
...
/*********************************************************************/
void CACEdit::SetEditSel(int nStartChar , int nEndChar)
{
	if(m_iType == _EDIT_)
		{
		((CEdit *)this)->SetSel(nStartChar,nEndChar);
		}
}
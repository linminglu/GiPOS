...
protected:
	...
	std::wstring				m_WindowName;			//!< Заголовок окна. Выводится в task bar. Если пустой, то в task bar не будет кнопки.
	HWND								m_hProgressWnd;		//!< Progress bar. Выводиться внизу окна. Подробнее см. #SetProgress.
	HWND								m_hParentWnd;
	std::wstring				m_ProgressMsg;		//!< Кэш сообщения о ходе процесса.
...
public:
  ...
  void SetImage(HMODULE hModule, UINT nResourceID);	//!< Задать изображение через идентификатор ресурса. Загрузка идет из секции "IMAGE".
	void SetWindowName(LPCWSTR windowName);		//!< Задать заголовок окна. Подробнее см. #m_WindowName.
